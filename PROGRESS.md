...
- **التغييرات:**
  1. **إعادة ترتيب عناصر القائمة الجانبية:** تم إعادة ترتيب القائمة وفق الهيكل المطلوب (لوحة المعلومات → مركز المهام → إدارة كراسات الشروط → إدارة استفسارات المتنافسين → إدارة الفحص وتقييم العروض → المساعد الذكي → التقارير → الدعم الفني → الإعدادات).
  2. **نقل إدارة اللجان وقاعدة المعرفة إلى الإعدادات:** تم نقلهما من القائمة الرئيسية إلى قسم الإعدادات الفرعي.
  3. **تحديث ملفات الترجمة:** تحديث المسميات العربية والإنجليزية لتتوافق مع المطلوب.
- **الملفات المعدلة:**
  - `frontend/src/config/navigation.ts` - إعادة هيكلة كاملة
  - `frontend/src/locales/ar.json` - تحديث مفاتيح الترجمة
  - `frontend/src/locales/en.json` - تحديث مفاتيح الترجمة
- **النشر:** ✅ تم البناء والنشر بنجاح على https://netaq.pro
- **Commit:** `feat: restructure sidebar menu to match required layout`
...
- **الحالة:** ✅ مكتمل
- **ما تم إنجازه:**
  - **تهيئة بيانات الصلاحيات:** استدعاء endpoint `/api/permission-matrix/seed` لتهيئة 810 قاعدة صلاحيات افتراضية تغطي 3 نطاقات (عام: 16 قاعدة، المنافسات: 170 قاعدة، اللجان: 4 قواعد) × عدد الأدوار
  - **إصلاح فلاتر Select Binding:** HTML `<select>` كان يعيد قيم string بدلاً من number مما يمنع الفلترة من العمل. تم إضافة `@change` handlers مع `parseInt()` للتحويل الصحيح
  - **تحسين عرض الفلاتر الديناميكية:** فلتر المرحلة يظهر فقط عند اختيار نطاق المنافسات، وفلتر أدوار اللجان يظهر فقط عند اختيار نطاق اللجان
- **نتائج الاختبار المباشر (Live Testing):**
  - ✅ فلتر النطاق (عام): يعرض 16 قاعدة فقط
  - ✅ فلتر النطاق (المنافسات): يعرض 170 قاعدة فقط
  - ✅ فلتر النطاق (اللجان): يعرض 4 قواعد فقط
  - ✅ فلتر المرحلة (التحليل الفني): يعرض 16 قاعدة ضمن المنافسات
  - ✅ تعديل الصلاحيات: تغيير checkbox يُحدّث عداد التغييرات
  - ✅ حفظ التغييرات: يحفظ بنجاح ويُحدّث عداد القواعد المخصصة
  - ✅ منح/سحب الكل وإعادة التعيين للافتراضي تعمل بشكل صحيح
- **الملفات المعدلة:**
  - `frontend/src/views/permissions/PermissionsMatrixView.vue` - إصلاح فلاتر select binding
- **النشر:** ✅ تم بناء ونشر الـ Frontend بنجاح على https://netaq.pro
- **Commit:** `fix: fix scope/phase/committee filter binding in PermissionsMatrixView`
