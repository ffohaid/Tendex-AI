...
```text
أنت وكيل تطوير (Developer Agent) تعمل على مشروع Tendex AI.
مهمتك هي TASK-908: الاختبار الشامل للمنصة الحية (Live Testing)، إدخال بيانات حقيقية، واختبار الذكاء الاصطناعي، مع المعالجة الفورية لجميع المشاكل والنواقص.

**الهدف الأساسي:**
التأكد من بناء جميع المتطلبات وترابط المكونات بجميع أنواعها، وأن المنصة تحقق أهدافها بشكل كامل وفق أفضل المعايير المعتمدة في وثائق المشروع الرئيسية. يجب عليك إجراء اختبارات صارمة وشاملة، وتجربة كل أنواع السيناريوهات ببيانات حقيقية، ومعالجة أي مشكلة أو نقص تجده مباشرة دون الرجوع للمستخدم.
...
**التعليمات الصارمة للاختبار والمعالجة:**
1. **الاختبار الحي المباشر (Live Testing):**
   - قم بتسجيل الدخول إلى المنصة الحية (netaq.pro) باستخدام الحساب المرفق.
   - قم بالمرور على **جميع** الصفحات والقوائم (كراسات الشروط، اللجان، التقييم الفني والمالي، مسارات الاعتماد، الاستفسارات، التقارير، المساعد الذكي، لوحة المشغل، الإعدادات).
2. **إدخال بيانات حقيقية كاملة:**
   - لا تستخدم بيانات وهمية (Test1, asdf). قم بإدخال بيانات حقيقية ومنطقية تتوافق مع سياق المشتريات الحكومية وكراسات الشروط.
   - تأكد من تأثير إدخال البيانات وتأكيد ظهورها في الجداول والتقارير والصفحات المرتبطة.
3. **اختبار الذكاء الاصطناعي (AI Testing):**
   - قم بإضافة مفتاح Gemini API المرفق في إعدادات الذكاء الاصطناعي في قاعدة البيانات (جدول AiConfiguration) أو عبر لوحة المشغل.
   - اختبر المساعد الذكي، توليد كراسات الشروط، وتحليل العروض بشكل كامل للتأكد من عملها بكفاءة عالية وباللغة العربية.
4. **المعالجة الفورية (Immediate Fixing):**
   - **مهم جداً:** خلال عملية الاختبار، أي مشكلة تواجهك (مثل أخطاء API 404/500، مشاكل في الترجمة i18n مثل `tenants.*`، أخطاء في الواجهة، أو عدم ترابط المكونات)، **يجب عليك معالجتها فوراً** في الكود المصدري.
   - استخدم صلاحيات السيرفر و GitHub لسحب الكود، إصلاح المشكلة (في الباك إند .NET أو الفرونت إند Vue)، بناء المشروع، وإعادة النشر على السيرفر مباشرة.
   - لا تتوقف عند اكتشاف الخطأ، بل أصلحه وأكمل الاختبار.
5. **التحقق من الجودة والترابط:**
   - تأكد من أن جميع الخصائص تعمل وفق المتطلبات الموجودة في وثائق المشروع الرئيسية (PRD v6، Architecture، إلخ).
   - تأكد من جودة الكود، الأداء، وتجربة المستخدم (UI/UX) ودعم اللغة العربية (RTL).

**خطوات العمل الإلزامية:**
1. اتصل بالسيرفر وتأكد من حالة الحاويات (Docker).
2. ابدأ جلسة اختبار شاملة على المنصة الحية باستخدام المتصفح.
3. عند إيجاد أي خلل (وقد تم رصد أخطاء API مسبقاً في صفحات مثل كراسات الشروط، اللجان، التقارير، ولوحة المشغل، بالإضافة لمشاكل الترجمة)، قم بتعديل الكود محلياً أو على السيرفر لإصلاحها.
4. أضف مفتاح Gemini وجرب ميزات الذكاء الاصطناعي.
5. ارفع جميع التعديلات والإصلاحات إلى GitHub.
6. أعد نشر التحديثات على السيرفر وتأكد من اختفاء المشاكل.
7. قم بتحديث `PROGRESS.md` بتقرير مفصل عن كل ما تم اختباره وإصلاحه.

**تنبيه:** هذا البرومبت يتطلب منك العمل كمهندس جودة (QA) ومطور (Full-stack Developer) ومدير عمليات (DevOps) في نفس الوقت. لا تطلب مساعدة، استخدم الصلاحيات الممنوحة لك لإنجاز المهمة بنسبة 100%.
```
